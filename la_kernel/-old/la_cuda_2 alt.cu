...
__device__ void compute_dot(vec_t* queries[kBlockSizeQ], vec_t* keys,
                            scalar_t out[kBlockSizeQ][kBlockSizeK], int64_t K) {
  ...
  for (int64_t k = 0; k < K / kVecSize; k += 1) {
    ...
    for (int64_t k_item_idx = 0; k_item_idx < kBlockSizeK; k_item_idx++) {
      ...
      for (int64_t q_item_idx = 0; q_item_idx < kBlockSizeQ; q_item_idx++) {
        sputnik::VectorCompute<vec_t>::Dot(q_i[q_item_idx], k_i,
                                           &out[q_item_idx][k_item_idx]);
      }
    }
  }
}
...
__device__ void compute_final_mult(
    vec_t* vi, scalar_t s_delta[kBlockSizeQ][kBlockSizeK],
...
#pragma unroll
      for (int64_t q_item_idx = 0; q_item_idx < kBlockSizeQ; q_item_idx++) {
        sputnik::VectorCompute<vec_t>::FMA(s_delta[q_item_idx][k_item_idx],
                                           tmp2, &buffer[q_item_idx][k]);
      }
    }
  }
}
...
__device__ __forceinline__ void compute_max(
    scalar_t a[kBlockSizeQ][kBlockSizeK], scalar_t b[kBlockSizeQ],
...
__device__ __forceinline__ void compute_and_update_scaling_coeffs(
    scalar_t m_i[kBlockSizeQ], scalar_t m_prime[kBlockSizeQ],
    scalar_t s_prime[kBlockSizeQ], scalar_t si[kBlockSizeQ][kBlockSizeK],
    scalar_t m_delta[kBlockSizeQ], scalar_t s_delta[kBlockSizeQ][kBlockSizeK]) {
#pragma unroll
  for (int64_t q_item_idx = 0; q_item_idx < kBlockSizeQ; q_item_idx++) {
...
template <typename scalar_t, typename vec_t, int kBlockSizeK, int kBlockSizeQ>
__device__ void add_attn_bias(scalar_t si[kBlockSizeQ][kBlockSizeK],
...
__device__ void compute_loop(
    vec_t* query_block[kBlockSizeQ], vec_t* key_i, vec_t* value_i,
    scalar_t m_prime[kBlockSizeQ], scalar_t s_prime[kBlockSizeQ],
    vec_t buffer[kBlockSizeQ][BUFFER_SIZE] /*TODO [BUFFER_SIZE limitation]*/,
    int64_t K, scalar_t* attn_bias_i,
    int64_t global_offset, int64_t N, scalar_t p, int64_t col_offset) {
  // si 是小矩阵
  scalar_t si[kBlockSizeQ][kBlockSizeK] = {0};
  compute_dot<scalar_t, vec_t, kBlockSizeK, kBlockSizeQ>(query_block, key_i, si,
                                                         K);
...
template <bool first, typename scalar_t, typename vec_t, int kBlockSizeK,
          int kBlockSizeQ, int BUFFER_SIZE, int WARP_SIZE>
struct UnrollLoop {
  static __device__ __forceinline__ void eval(
      vec_t* query_block[kBlockSizeQ], at::TensorAccessor<scalar_t, 2> key,
      at::TensorAccessor<scalar_t, 2> value, scalar_t m_prime[kBlockSizeQ],
      scalar_t s_prime[kBlockSizeQ],
      vec_t buffer[kBlockSizeQ][BUFFER_SIZE] /*TODO [BUFFER_SIZE limitation]*/,
      int64_t K, int64_t N, at::TensorAccessor<scalar_t, 2> attn_bias,
      int64_t global_offset, scalar_t p) {
    ...
    // if (l < end_iter) {
    {
      for (; l < end_iter; l += step) {
        ...
        compute_loop<scalar_t, vec_t, kBlockSizeK, kBlockSizeQ, BUFFER_SIZE>(
            ...
            attn_bias_i, global_offset, N, p, l);
      }
    }
    {
      UnrollLoop<false, scalar_t, vec_t, kBlockSizeK / 2, kBlockSizeQ,
                 BUFFER_SIZE, WARP_SIZE>::eval(query_block, key, value, m_prime,
                                               s_prime, buffer, K, N, attn_bias,
                                               global_offset, p);
    }
  }
};

template <bool first, typename scalar_t, typename vec_t, int kBlockSizeQ,
          int BUFFER_SIZE, int WARP_SIZE>
struct UnrollLoop<first, scalar_t, vec_t, 0, kBlockSizeQ, BUFFER_SIZE,
                  WARP_SIZE> {
  static __device__ __forceinline__ void eval(
      vec_t* query_block[kBlockSizeQ], at::TensorAccessor<scalar_t, 2> key,
      at::TensorAccessor<scalar_t, 2> value, scalar_t m_prime[kBlockSizeQ],
...
__global__ void attention_kernel(
  ...
  constexpr int kVecSize = sizeof(vec_t) / sizeof(scalar_t);
  static_assert(integerIsPowerOf2(kBlockSizeK * WARP_SIZE),
  ...
  UnrollLoop<true, scalar_t, vec_t, kBlockSizeK, kBlockSizeQ, BUFFER_SIZE,
             WARP_SIZE>::eval(query_block, key[batch_idx], value[batch_idx],
                              m_prime, s_prime, buffer, K, N,
                              attn_bias[batch_idx], global_offset,
                              p);
...
at::PackedTensorAccessor<scalar_t, 3> _packed_tensor_accessor_or_dummy(
  ...
  if (attn_bias.defined()) {
    ...
    return at::PackedTensorAccessor<scalar_t, 3>(nullptr, zeros.data(),
                                                 zeros.data());
  }
}

template <bool compute_logsumexp=false>
void launch_attention(at::Tensor& res,
  ...
  if ((K % 4) == 0) {
    TORCH_CHECK(K / 4 <= BUFFER_SIZE,
                "For now only a certain number of K values are supported. Let "
                "us know if you hit this and we will fix it");
    attention_kernel<scalar_t, float4, kBlockSizeK, kBlockSizeQ, WARP_SIZE,
                     BUFFER_SIZE>
        <<<grid, block, 0, stream>>>(res.packed_accessor<scalar_t, 3>(),
                                     query.packed_accessor<scalar_t, 3>(),
                                     key.packed_accessor<scalar_t, 3>(),
                                     value.packed_accessor<scalar_t, 3>(),
                                     attn_bias_packed);
  } else if ((K % 2) == 0) {
    TORCH_CHECK(K / 2 <= BUFFER_SIZE,
                "For now only a certain number of K values are supported. Let "
                "us know if you hit this and we will fix it");
    attention_kernel<scalar_t, float2, kBlockSizeK, kBlockSizeQ, WARP_SIZE,
                     BUFFER_SIZE>
        <<<grid, block, 0, stream>>>(res.packed_accessor<scalar_t, 3>(),
                                     query.packed_accessor<scalar_t, 3>(),
                                     key.packed_accessor<scalar_t, 3>(),
    ...
    TORCH_CHECK(K <= BUFFER_SIZE,
                "For now only a certain number of K values are supported. Let "
                "us know if you hit this and we will fix it");
    attention_kernel<scalar_t, float, kBlockSizeK, kBlockSizeQ, WARP_SIZE,
                     BUFFER_SIZE>
        <<<grid, block, 0, stream>>>(res.packed_accessor<scalar_t, 3>(),
                                     query.packed_accessor<scalar_t, 3>(),
                                     key.packed_accessor<scalar_t, 3>(),
...
std::tuple<at::Tensor, at::Tensor> attention(
    ...
    bool compute_logsumexp, const c10::optional<at::Tensor>& attn_bias_,
    double p) {
  ...
  launch_attention<false>(res, query, key, value, attn_bias);
...
template <typename scalar_t, typename vec_t, int kBlockSizeQ, int kBlockSizeK,
          int TILE_SIZEQ, int TILE_SIZEK, bool check_bounds>
__global__ void attention_backward_kernel(
    ...
    at::PackedTensorAccessor<scalar_t, 3> attn_bias, scalar_t p) {
  int64_t K = query.size(2);
  int64_t B = query.size(0);
  int64_t M = query.size(1);
  int64_t N = key.size(1);
  ...
  for (int k_item_idx = 0; k_item_idx < kBlockSizeK; k_item_idx++) {
    ...
    if (check_bounds) index = min(index, N - 1);
  ...
  for (int q_item_idx = 0; q_item_idx < kBlockSizeQ; q_item_idx++) {
    ...
    if (check_bounds) index = min(index, M - 1);
  ...
  for (int64_t k = 0; k < K / kVecSize; k += 1) {
    for (int k_item_idx = 0; k_item_idx < kBlockSizeK; k_item_idx++) {
      ...
      for (int q_item_idx = 0; q_item_idx < kBlockSizeQ; q_item_idx++) {
        sputnik::VectorCompute<vec_t>::Dot(__ldg(qb[q_item_idx] + k), kk,
                                           &attn_v[q_item_idx][k_item_idx]);
        sputnik::VectorCompute<vec_t>::Dot(
            __ldg(gb[q_item_idx] + k), tt,
    ...
    for (int64_t i = 0; i < TILE_SIZEQ; i++) {
      ...
      for (int k_item_idx = 0; k_item_idx < kBlockSizeK; k_item_idx++) {
        sputnik::VectorCompute<vec_t>::FMA(
            fact[i][kBlockSizeK * threadIdx.y + k_item_idx], kk,
            &res[k_item_idx]);
      }
    }
    for (int k_item_idx = 0; k_item_idx < kBlockSizeK; k_item_idx++) {
  ...
  for (int64_t k = threadIdx.y; k < K / kVecSize; k += blockDim.y) {
    ...
    for (int64_t i = 0; i < TILE_SIZEK; i++) {
      ...
      for (int q_item_idx = 0; q_item_idx < kBlockSizeQ; q_item_idx++) {
        sputnik::VectorCompute<vec_t>::FMA(
            fact[kBlockSizeQ * threadIdx.x + q_item_idx][i], kk,
            &res[q_item_idx]);
      }
    }
    for (int q_item_idx = 0; q_item_idx < kBlockSizeQ; q_item_idx++) {
...
void launch_attention_backward(at::Tensor& grad_q, at::Tensor& grad_k,
                               at::Tensor& grad_v, const at::Tensor& grad_out,
                               const at::Tensor& query, const at::Tensor& key,
                               const at::Tensor& value,
                               const at::Tensor& logsumexp,
                               const at::Tensor& output,
                               const at::Tensor& attn_bias, float p,
  ...
  dim3 grid(ceil_div(M, int64_t(TILE_SIZEQ)), ceil_div(N, int64_t(TILE_SIZEK)),
            B);
  ...
  if ((M % TILE_SIZEQ == 0) && (N % TILE_SIZEK == 0)) {
    attention_backward_kernel<scalar_t, vec_t, kBlockSizeQ, kBlockSizeK,
                              TILE_SIZEQ, TILE_SIZEK, false>
        <<<grid, block, 0, stream>>>(grad_q.packed_accessor<scalar_t, 3>(),
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> attention_backward(
    ...
    const at::Tensor& value, const at::Tensor& logsumexp,
    const at::Tensor& output, const c10::optional<at::Tensor>& attn_bias_,
    double p, int64_t rng_seed, int64_t rng_offset) {
  ...
  if ((K % 4) == 0) {
    launch_attention_backward<float, float4>(
        grad_q, grad_k, grad_v, grad_out, query, key, value, logsumexp, output,
        attn_bias);
  } else if ((K % 2) == 0) {
...
template <bool first, typename scalar_t, typename vec_t, int kBlockSizeK,
          int kBlockSizeQ, int WARP_SIZE>
struct UnrollLoopForMask {
  static __device__ __forceinline__ void eval(scalar_t* output[kBlockSizeQ],
                                              int64_t N, int64_t M,
    ...
    // if (l < end_iter) {
    {
      for (; l < end_iter; l += step) {
        ...
        for (int jj = 0; jj < kBlockSizeQ; jj++) {
          for (int kk = 0; kk < kBlockSizeK; kk++) {
            if (query_idx + jj < M) output[jj][l + kk] = s_delta[jj][kk];
          }
        }
      }
    }
    {
      UnrollLoopForMask<false, scalar_t, vec_t, kBlockSizeK / 2, kBlockSizeQ,
                        WARP_SIZE>::eval(output, N, M,
                                         global_offset, p);
    }
  }
};
...
struct UnrollLoopForMask<first, scalar_t, vec_t, 0, kBlockSizeQ, WARP_SIZE> {
  static __device__ __forceinline__ void eval(scalar_t* s_delta[kBlockSizeQ],
                                              int64_t N, int64_t M,
                                              int64_t global_offset,
                                              scalar_t p) {}
};
...
TORCH_LIBRARY_IMPL(xformers, CUDA, m) {
  m.impl(TORCH_SELECTIVE_NAME("xformers::efficient_attention_forward_small_k"),
         TORCH_FN(attention));
  m.impl(TORCH_SELECTIVE_NAME("xformers::efficient_attention_backward_small_k"),
