/*
不要使用torch::命名空间 和 #include <torch/extension.h> 
因为 torch:: 命名空间会 引入一大堆依赖
建议只使用 aten 的 at:: 空间，这样只需要加入 pytorch 基础的头文件就可以编译了
缺点是不能使用 pytorch 的一些高级功能，例如 nn 库里面的模组。
*/
/*
...
template <
	...
	typename scalar_t
>
__host__ __device__ inline void my_matmul_par(
	scalar_t pM1[m1_row][m1_col],
	scalar_t pM2[m2_row][m2_col],
	scalar_t pMO[o_row][o_col],
	int cur_thread_y,
...
__host__ __device__ inline void mul_arr_par(
...
template <int row, int col, typename scalar_t>
__host__ __device__ inline void add_arr_par(
	scalar_t src1[row][col],
	scalar_t src2[row][col],
	scalar_t dst[row][col],
	int cur_thread_y,
...
__host__ __device__ inline void atomic_add_arr_par(
	...
	for (int i=cur_thread_y; i<row; i+=thread_block_h)
		...
		{
			if (dst_to_src)
				atomicAdd(&src[i][j], dst[i][j]);
			else
				atomicAdd(&dst[i][j], src[i][j]);
		}
}
...
template <int row, int col, typename scalar_t>
__host__ __device__ inline void copy_arr_par(
	scalar_t src[row][col],
	scalar_t dst[row][col],
	int cur_thread_y,
	int cur_thread_x,
...
template <int row, int col, typename scalar_t>
__host__ __device__ inline void zero_arr_par(
	scalar_t dst[row][col],
	int cur_thread_y,
	int cur_thread_x,
	int thread_block_h,
